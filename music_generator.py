...
def round_cutoff(num, cutoff=0.06):
    """
...
def create_midi(savefile, notes, durations):
    offset = 0
    output_notes = []
...
def generate_music(duration_model: Sequential, notes_model: Sequential, seed_song, song_length)->(list,list):
    ...
    # Temperature based randomization of outputs
    temperature = 100.0
    while sum(song_durations) < song_length:
        ...
        number_of_notes = np.apply_along_axis(round_cutoff, 0, predicted_notes)
        number_of_notes = int(sum(number_of_notes))
        predicted_notes = predicted_notes[0]
        predicted_notes /= temperature
        ...
        song_notes.append(random_pred_notes)
        ...
        random_pred_notes.append(predicted_duration)
        pattern.append(np.array(random_pred_notes))
...
if __name__ == "__main__":
    ...
    while option < 2:
        ...
        seed_song_file = 'midi_songs/testing/mz_311_1.mid' # user_input.get_user_filename('Path to seed song', '.mid')
        ...
        notes_model_file = 'models/notes/note-model-19-0.0248.hdf5' # user_input.get_user_filename('Path to notes model', '.hdf5')
        save_filename = 'songs/song2-mz'  # user_input.get_user_filename('Path to notes model', '.hdf5')
        ...
        seed_song = np.array([load_song(seed_song_file)[51:51+sequence_length]])

        # generate music
        notes, durations = generate_music(duration_model, notes_model, seed_song, length)
        create_midi(save_filename, notes, durations)
